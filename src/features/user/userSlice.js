...
export const deleteCartProduct = createAsyncThunk(
  ...
  async (inforItem, thunkAPI) => {
    try {
      return await authService.removePoductFromCart(inforItem);
    } catch (error) {
      return thunkAPI.rejectWithValue(error);
    }
  }
...
export const updateCartProduct = createAsyncThunk(
  ...
  async (cartDetail, thunkAPI) => {
    try {
      return await authService.updateProdCart(cartDetail);
    } catch (error) {
      return thunkAPI.rejectWithValue(error);
    }
  }
...
export const getLocation = createAsyncThunk(
  ...
  async (thunkAPI) => {
    try {
      return await authService.getApiLocation();
    } catch (error) {
      return thunkAPI.rejectWithValue(error);
    }
  }
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(addProdToCart.fulfilled, (state, action) => {
        ...
        state.isError = false;
        state.isSuccess = true;
        state.userCartChange = action.payload;
        if (state.isSuccess) {
      ...
      .addCase(resetPass.fulfilled, (state, action) => {
        ...
        if (state.isSuccess) {
          toast.success('password upadte successfully');
        }
      })
      .addCase(resetPass.rejected, (state, action) => {
        ...
        state.message = action.error;
        if (state.isError === true) {
          toast.error('password upadte error');
        }
      });
  },
});
